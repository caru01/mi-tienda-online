...
const TABS = [
  {
    ...
    emoji: '📥',
    color: 'yellow',
    statuses: ['por_aceptar'],
    emptyMsg: 'Aquí verás nuevas órdenes de tus clientes. Acéptalas para confirmar que las prepararás.',
  },
  {
    key: 'en_preparacion',
    label: 'PREPARAR',
    emoji: '👨‍🍳',
    color: 'blue',
    statuses: ['en_preparacion', 'preparando'],
    emptyMsg: 'Aquí verás las órdenes a preparar de tu lado.',
  },
  {
    key: 'por_entregar',
    label: 'ENTREGAR',
    emoji: '🛵',
    color: 'green',
    statuses: ['por_entregar', 'en_camino'],
    emptyMsg: 'Aquí verás órdenes listas de tu lado que los Domiciliarios estarán listos para entregar.',
  },
]
...
export default function OrdersTab({
  ...
  // Mobile uses tab-switching (single column), desktop shows all 3 columns
  const [mobileTab, setMobileTab] = useState('por_aceptar')
  ...
  const handlePrint = (sale: any) => {
    ...
    win.document.write(`
      ...
      <hr/>
      <p><span class="label">Pedido:</span></p><pre>${sale.order_detail}</pre>
      ...
      <div class="total">$ ${Number(sale.total_amount).toLocaleString()}</div><hr/>
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between mb-4 px-1">
        ...
        <button onClick={onRefresh} className="text-xs text-gray-400 hover:text-white bg-white/5 px-3 py-1.5 rounded-lg transition-colors">
          ↺ Actualizar
        </button>
      </div>

      {/* ═══════════════════════════════════════
          MÓVIL: Tab bar + columna única (igual que antes)
          ═══════════════════════════════════════ */}
      <div className="md:hidden flex flex-col flex-1">
        {/* Tab bar móvil */}
        <div className="flex w-full mb-4 rounded-2xl overflow-hidden border border-white/10 bg-black/30">
          {TABS.map(tab => {
            const isActive = tab.key === mobileTab
            const count = countFor(tab)
            const colorText = tab.color === 'yellow' ? 'text-yellow-400' : tab.color === 'blue' ? 'text-blue-400' : 'text-green-400'
            const colorBorder = tab.color === 'yellow' ? 'border-yellow-400' : tab.color === 'blue' ? 'border-blue-400' : 'border-green-400'
            const colorBg = tab.color === 'yellow' ? 'bg-yellow-400' : tab.color === 'blue' ? 'bg-blue-400' : 'bg-green-400'
            return (
              <button
                key={tab.key}
                onClick={() => setMobileTab(tab.key)}
                className={`flex-1 py-4 flex flex-col items-center gap-1 font-black text-xs tracking-widest transition-all relative ${
                  isActive ? `${colorText} bg-white/10 border-b-2 ${colorBorder}` : 'text-gray-500 hover:text-gray-300 hover:bg-white/5'
                }`}
              >
                <span className="text-xl">{tab.emoji}</span>
                <span>{tab.label}</span>
                {count > 0 && (
                  <span className={`absolute top-2 right-2 w-5 h-5 flex items-center justify-center rounded-full text-xs font-bold ${isActive ? `${colorBg} text-black` : 'bg-white/20 text-white'}`}>
                    {count}
                  </span>
                )}
              </button>
        ...
        {/* Lista de órdenes — columna activa en móvil */}
        <div className="space-y-3 overflow-y-auto flex-1 pb-4">
          {(() => {
            const activeTabObj = TABS.find(t => t.key === mobileTab)!
            const orders = filteredFor(activeTabObj)
            if (orders.length === 0) return (
              <div className="flex flex-col items-center justify-center py-16 text-gray-600 text-center px-6">
                <Package size={48} className="mb-4 opacity-30" />
                <p className="text-sm font-bold text-gray-500">{activeTabObj.emptyMsg}</p>
              </div>
            )
            return orders.map((sale: any) => (
              <OrderCard
                key={sale.id}
                sale={sale}
                tab={mobileTab}
      ...
      <div className="hidden md:flex gap-4 flex-1 min-h-0">
        {TABS.map(tab => {
          ...
          return (
            <div key={tab.key} className="flex flex-col flex-1 min-w-0">
              <div className={`flex items-center justify-between px-4 py-3 rounded-2xl border mb-3 ${headerColor}`}>
                ...
                {count > 0 && (
                  <span className="text-xs font-black px-2.5 py-0.5 rounded-full bg-black/30">{count}</span>
                )}
              </div>
              <div className="space-y-3 overflow-y-auto flex-1 pr-0.5">
                {orders.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center px-4 rounded-2xl border border-dashed border-white/5">
                    <Package size={32} className="mb-2 opacity-20" />
                    <p className="text-xs font-bold text-gray-600 leading-relaxed">{tab.emptyMsg}</p>
                  </div>
                ) : (
                  orders.map((sale: any) => (
                    <OrderCard
...
function OrderCard({ sale, tab, loading, isNew, now, onStatus, onPrint, onDismissNew }: {
  ...
  const minutesAgo = isValidDate ? Math.floor((now - safeDate.getTime()) / 60000) : 0
  const minuteLabel = minutesAgo < 1 ? 'Ahora' : minutesAgo === 1 ? '1 min' : `${minutesAgo} min`
  const isTransfer = sale.payment_method === 'transferencia'

  const newGlow = isNew
    ? 'shadow-[0_0_20px_rgba(34,197,94,0.5)] border-green-400/60 animate-pulse'
    : minutesAgo >= 20 ? 'border-red-500/40'
    : minutesAgo >= 10 ? 'border-yellow-500/30'
    : 'border-white/10'

  const bgGradient = isNew
    ? 'bg-gradient-to-b from-green-500/10 to-black/50'
    : tab === 'por_aceptar' ? 'bg-gradient-to-b from-yellow-400/5 to-black/40'
    : tab === 'en_preparacion' ? 'bg-gradient-to-b from-blue-400/5 to-black/40'
    : 'bg-gradient-to-b from-green-400/5 to-black/40'

  return (
    <div
      onClick={() => isNew && onDismissNew(sale.id)}
      className={`rounded-2xl overflow-hidden border transition-all ${newGlow} ${bgGradient}`}
    >
      {/* Cabecera */}
        ...
        {tab === 'por_aceptar' && (
          <button
            ...
            className={`w-full py-2.5 rounded-xl font-black text-sm tracking-wide transition-all active:scale-95 ${
              isTransfer ? 'bg-distrito-accent text-black hover:bg-yellow-300' : 'bg-green-500 text-black hover:bg-green-400'
            } disabled:opacity-50`}
          >
